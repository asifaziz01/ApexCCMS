...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && courseDetailMatch) {
      ...
      if (courseDetailMatch[2] === 'versions') return json(res, 200, { courseId: course.id, versions: [{ id: `${course.id}-V1.0`, version: 'v1.0', status: 'Historical', effectiveTerm: 'Fall 2025', title: course.title }, { id: `${course.id}-V${course.version.replace('v','')}`, version: course.version, status: course.status === 'Active' ? 'Official' : 'Proposed', effectiveTerm: course.effectiveTerm, title: course.title }] });
