...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && courseDetailMatch) {
      ...
      if (courseDetailMatch[2] === 'versions') { const versions = course.status === 'Active' ? [{ id: `${course.id}-V1.0`, version: 'v1.0', status: 'Historical', effectiveTerm: 'Fall 2025', title: course.title }, { id: `${course.id}-V${course.version.replace('v','')}`, version: course.version, status: 'Official', effectiveTerm: course.effectiveTerm, title: course.title }] : [{ id: `${course.id}-V1.0`, version: 'v1.0', status: 'Historical', effectiveTerm: 'Fall 2025', title: course.title }, { id: `${course.id}-V2.0`, version: 'v2.0', status: 'Official', effectiveTerm: 'Fall 2025', title: course.title }, { id: `${course.id}-V${course.version.replace('v','')}`, version: course.version, status: 'Proposed', effectiveTerm: course.effectiveTerm, title: course.title }]; return json(res, 200, { courseId: course.id, versions }); }
